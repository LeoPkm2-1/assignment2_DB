...
function Order() {
  ...
  return orderList.length ? (<StyledOrder>
      <StyledOrderList>
        <h3>Order List</h3>
      ...
      <StyledProductOrderList>
        <h3>Đơn hàng {moment(orderList.find(o => o?.order_id === selectOrder)?.order_date).format('HH:MM DD-mm-yyyy')}</h3>
        <Table columns={columnsProduct({action1: () => {}, action2: () => {}}, [])} dataSource={orderItem}/>
      </StyledProductOrderList>
    </StyledOrder>)
    : <img width={400} src='https://organickle.com/images/no-order.svg' alt=''/>
}
